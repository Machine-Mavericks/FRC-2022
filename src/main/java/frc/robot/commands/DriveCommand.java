...
public class DriveCommand extends CommandBase {
  ...
  public void execute() {
    ...
    double rotInput = 0;//OI.driverController.getRightX();

    xInput = Math.abs(xInput) > 0.1 ? xInput : 0;
    yInput = Math.abs(yInput) > 0.1 ? yInput : 0;

    m_drivetrain.drive(new Translation2d(xInput*Drivetrain.MAX_VELOCITY_METERS_PER_SECOND, yInput*Drivetrain.MAX_VELOCITY_METERS_PER_SECOND), rotInput*Drivetrain.MAX_ANGULAR_VELOCITY_RADIANS_PER_SECOND, false);
  }
