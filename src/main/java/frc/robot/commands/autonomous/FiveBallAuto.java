...
public class FiveBallAuto extends SequentialCommandGroup {
  ...
  public FiveBallAuto() {
    ...
    addCommands(
      ...
      new AutoDriveToPose(new Pose2d(new Translation2d(-1.70, 0.69), Rotation2d.fromDegrees(-147.2)), 0.35, 0.15, 10.0),
      // Intake the ball
      ...
      new InstantCommand(()-> RobotContainer.m_shooter.setShooterSpeed(4650.0)),  
      // Intake the ball
      new SteerTowardsBall(true, 4.0, 0.2),
      // Drive back a bit
      new AutoDriveToPose(new Pose2d(new Translation2d(-6.12, -0.165), Rotation2d.fromDegrees(-158.0)), 0.3, 0.10, 15.0),
      // Shoot first ball
      new AutoShootCommand(AutoShootCommand.HIGH_SPEED).deadlineWith(new SteerTowardsHub()),
      // Shoot first ball
      new AutoShootCommand(AutoShootCommand.HIGH_SPEED).deadlineWith(new SteerTowardsHub())
